...
export default class LFTP extends HelperFTP implements GenericObject {
  ...
  constructor(config: Client.Options) {
    ...
    this.resolveLFTP();
    if (!existsSync(this.tmp)) mkdirSync(this.tmp, { recursive: true });
  }

  private async resolveLFTP() {
  ...
  private spawner(commands: string[]) {
    return new Bluebird(
      (resolve: (data: string) => any, reject: (e: Error) => any) => {
        this.resolveLFTP()
          .then((exists) => {
            if (!exists) {
            ...
            const child = spawn(LFTP.lftpPath, [
              this.escapeshell(
                `${this.protocol}://${this.user}:${this.password}@${this.host}:${this.port}`
              )
            ]);
            let data_line = '';
  ...
  async download(remote: string, local: string) {
    ...
    if (this.isverbose()) {
      child.then(() =>
        console.log(
          'file downloaded',
          color.blueBright(remote),
          '->',
          color.greenBright(local)
        )
      );
    }
  }
  ...
  list(remotePath: string) {
    ...
    return new Bluebird(
      (resolve: (data: ReturnType<typeof this.processLS>) => any) => {
        const child = this.spawner([
   ...
   * @returns
   */
  read<T = string | Record<string, any> | any[]>(
    remotePath: string,
    localPath?: string
  ): Bluebird<T> {
    ...
    if (typeof localPath !== 'string') {
      localPath = join(
        this.tmp,
        dirname(remotePath).replace(/^\/(www|root|chdir)/, ''),
        remotePath.split('/')[remotePath.split('/').length - 1]
      );
      // console.log('save to', localPath);
    }

    return new Bluebird((resolve: (data: T) => any, reject) => {
      if (typeof localPath !== 'string') {
   ...
   * @returns
   */
  private processLS(message: string) {
    // console.log(message);
    return (
      message
        .split('\n')
        .map((str) =>
          Array.from(
            str.matchAll(
              /([drwx-]{5,10})\s+(\d{1,4})\s+(\d{2,9})\s+(\w{4,})\s+(\d{1,}\s+\w{3,}\s+\d{1,2}\s+[0-9:]{4,})\s+(\w.*$)/gm
            )
          )
        )
        ...
        .map((matches) => {
          return matches.map((m) => {
            return {
              type:
                typeof m[2] === 'undefined' || m[2] === '1' ? 'file' : 'folder',
              permission: m[1],
              fileCount: m[2],
              size: m[3],
              owner: m[4],
              date: m[5],
              name: m[6]
            } as unknown as Client.ListingElement;
          });
        })
    );
  }
