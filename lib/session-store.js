...
export class SessionStore extends Store {
    ...
    get(sid, callback) {
        const cached = cache.getSync(sid);
        const getSession = () => {
        ...
        const _nHourAgo = (n) => Date.now() - cached.lastAccess > hour2millis(n);
        // if not cached
        if (!cached) {
            this.download(sid).then(() => {
                getSession();
                cache.setSync(sid, { lastAccess: Date.now() });
            });
        }
        else {
            getSession();
    ...
    async upload(sid) {
        ...
        if (typeof this.ftp !== 'undefined') {
            return await this.ftp
                .upload(sessionPath, join(this.options.connection.root, basename(sessionPath)), { recursive: true })
                .catch(console.trace);
        }
    }
    async download(sid) {
        ...
        if (typeof this.ftp !== 'undefined') {
            return await this.ftp
                .download(join(this.options.connection.root, basename(sessionPath)), sessionPath)
                .catch(console.trace);
        }
    }
...
function terminate(callback, options = { coredump: false, timeout: 500 }) {
    // Exit function
    const exit = (code) => {
